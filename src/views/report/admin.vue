<template>
    <v-container class="px-0">
        <!-- <v-row justify="center">{{selectedReportType}}</v-row>
        <v-row justify="center">{{selectedQuiz}}</v-row> -->
    ...
    methods:{     
        ...
        chkQuizSelected(){
            if(this.isEmailAllowed==false){
                ...
                this.$root.$emit('message_from_parent_long','Your email settings not done yet! Please set those then sent email.')
        ...
        sentEmail(){
            if(this.$refs.form.validate()){
                ...
                const obj={
                    logoPath:config.hostname+this.$store.getters['settings/logoPath'].replace(/\\/g, '/'), //use this path when you host on live server
                ...
                this.$root.$emit('message_from_parent_long','Email sent successfully')
                this.dialogEmail=false
            }
        },
        submitAnswer(id){
