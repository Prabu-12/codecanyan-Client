...
export default {
    ...
    methods:{
        ...
        formatRunning(item){
            return item.isRunning==false?'':'On-live'
        },
        getMarkOptions(){
            ...
            .then(response=>{
                if(response.status==200){
                    if(this.emailInvitationToEmail==true){
                        var logoPath=config.hostname+this.$store.getters['settings/logoPath'].replace(/\\/g, '/')
                        var siteUrl=window.location.origin
                        var registrationLink=siteUrl
                        var body='<b>'+this.$store.getters['settings/siteTitle']+'</b> Admin is invited you to attened an Exam. If you not registered yet <a href="'+registrationLink+'">Click here</a> to register and attend the Exam.'
        ...
        chkPaymentAllowed(){
            if(this.allSettings.currency!='' && this.allSettings.currencySymbol!='' &&this.allSettings.stripeSecretKey!=''){
        ...
        save (){
            if(this.$refs.form.validate()){
                ...
                if(objTopic.quizPrice>0 && this.isPaymentAllowed==false){
                    ...
                    this.$root.$emit('message_from_parent_long','Your email settings not done yet! Please set those then sent invitation to email.')
